...
test("getReplayCollectionTiles returns only server replay-selected forms", () => {
  const state = createInitialState({
    diagonalAssist: false,
    rng: makeRng(21),
  });
  state.evolutionChoices.red[2] = "T2_RED_BLUE_A";
  state.evolutionChoices.red[3] = "T3_RED_BLUE_A";
  state.evolutionChoices.blue[4] = "T4_BLUE_RED_A";
  state.evolutionChoices.green[2] = "";

  assert.deepEqual(getReplayCollectionTiles(state), {
    T2_RED_BLUE_A: true,
    T3_RED_BLUE_A: true,
    T4_BLUE_RED_A: true,
  });
...
test("specialScore vibe grants score when special tiles form", () => {
  const base = createInitialState({ diagonalAssist: false, specialTiles: true, rng: makeRng(200), vibe: NEUTRAL_VIBE });
  const boosted = createInitialState({
    diagonalAssist: false,
    specialTiles: true,
    rng: makeRng(200),
    vibe: { ...NEUTRAL_VIBE, specialScore: 120 },
  });
  for (const state of [base, boosted]) {
    state.board = boardFromColorIds([
      ["blue", "green", "yellow", "purple", "cyan", "white", "black", "blue"],
      ["green", "yellow", "purple", "cyan", "white", "black", "blue", "green"],
      ["red", "red", "red", "red", "blue", "green", "yellow", "purple"],
      ["yellow", "purple", "cyan", "white", "black", "blue", "green", "yellow"],
      ["purple", "cyan", "white", "black", "blue", "green", "yellow", "purple"],
      ["cyan", "white", "black", "blue", "green", "yellow", "purple", "cyan"],
      ["white", "black", "blue", "green", "yellow", "purple", "cyan", "white"],
      ["black", "blue", "green", "yellow", "purple", "cyan", "white", "black"],
    ]);
  }

  const baseResult = resolveBoard(base.board, base, makeRng(201));
  const boostedResult = resolveBoard(boosted.board, boosted, makeRng(201));

  assert.equal(baseResult.cascadeSteps[0].specialSpawns[0].special, "cross");
...
test("trusted replay recomputes a submitted run from seed and action log", () => {
  ...
  const manual = createInitialState({
    diagonalAssist: false,
    diagonalSwaps: false,
    specialTiles: true,
    endlessRun: true,
...
test("getReplayResultSummary returns the server-comparable run result shape", () => {
  const { state } = replayRun(83, []);
  ...
  assert.equal(summary.score, state.score);
  assert.equal(summary.movesUsed, state.movesUsed);
});
