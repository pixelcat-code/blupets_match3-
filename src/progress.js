...
function progressStorageKey() {
  return currentProgressUser
    ? `${PROGRESS_STORAGE_PREFIX}:user:${currentProgressUser}`
    : `${PROGRESS_STORAGE_PREFIX}:guest`;
}

export function setProgressUser(userId) {
  currentProgressUser = userId ? String(userId) : null;
}
...
export const SARAI_HEART_QUEST_ID = "sarai_heart_quest";
export const SARAI_HEART_QUEST_TARGET = 11;
export const SARAI_HEART_QUEST_REWARD = 11;
...
// Check all milestone conditions against the current progress and award
// capsules for any newly met. Unlike foldRun(), does NOT update lifetime
// counters — safe to call after capsule draws or at startup so collection
// quests don't wait for the next run to pay out.
export function checkMilestoneCapsules(progress) {
  if (!progress.milestones) progress.milestones = { counters: emptyCounters(), unlocked: {} };
  if (!progress.milestones.unlocked) progress.milestones.unlocked = {};
  if (!progress.milestones.counters) progress.milestones.counters = emptyCounters();
  progress.capsules = Math.max(0, Math.floor(Number(progress.capsules) || 0));
  const collection = collectionStats(progress);
  const counters = progress.milestones.counters;
  const evalCtx = {
  ...
  for (const m of MILESTONE_BADGES) {
    if (!progress.milestones.unlocked[m.id] && m.test(evalCtx)) {
      ...
      const capsules = milestoneCapsuleReward(m.tier);
      progress.capsules += capsules;
      awarded.push({ id: m.id, name: m.label, capsules });
    }
  }
  if (awarded.length) save(progress);
  return awarded;
}
...
function normalizeSaraiHeartQuest(quest = {}) {
  const matches = Number.isFinite(Number(quest.matches))
    ? Math.max(0, Math.floor(Number(quest.matches)))
    : 0;
  const completed = Boolean(quest.completed) || matches >= SARAI_HEART_QUEST_TARGET;
  return {
    matches: Math.min(matches, SARAI_HEART_QUEST_TARGET),
    completed,
...
function normalizeProgress(parsed = {}) {
  ...
  return {
    ...
    saraiHeartQuest: normalizeSaraiHeartQuest(parsed.saraiHeartQuest),
...
export function loadProgress() {
  if (!currentProgressUser) return emptyProgress(); // guests never persist
  try {
...
function save(progress) {
  if (!currentProgressUser) return; // guests: no localStorage writes
  try {
    globalThis.localStorage?.setItem?.(progressStorageKey(), JSON.stringify(normalizeProgress(progress)));
  } catch {
    // Storage can be unavailable in private modes or tests; keep in-memory state.
  }
  return progress;
}
...
export function getSaraiHeartQuest(progress) {
  return normalizeSaraiHeartQuest(progress?.saraiHeartQuest);
}

export function recordSaraiHeartMatches(progress, matches) {
  const amount = Math.max(0, Math.floor(Number(matches) || 0));
  const current = normalizeSaraiHeartQuest(progress?.saraiHeartQuest);
  if (amount <= 0 || current.completed) {
    progress.saraiHeartQuest = current;
    return { matched: 0, quest: current, completedNow: false, reward: 0 };
  }

  const nextMatches = Math.min(SARAI_HEART_QUEST_TARGET, current.matches + amount);
  const completedNow = nextMatches >= SARAI_HEART_QUEST_TARGET;
  const reward = completedNow && !current.rewarded ? SARAI_HEART_QUEST_REWARD : 0;
  const quest = {
    matches: nextMatches,
    completed: current.completed || completedNow,
    rewarded: current.rewarded || reward > 0,
  };
  progress.saraiHeartQuest = quest;
  if (reward > 0) {
    progress.capsules = Math.max(0, Math.floor(Number(progress.capsules) || 0)) + reward;
  }
  save(progress);
  return { matched: amount, quest, completedNow, reward };
}
